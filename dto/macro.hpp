...
#define FROM_JSON_STR_OPT(j, u, name) u.name = j.contains(#name) && j[#name].is_string()? std::make_optional(j.at(#name).get<std::string>()):std::nullopt

#define FROM_JSON_INT_OPT(j, u, name) u.name = j.contains(#name) && j[#name].is_number_integer()? std::make_optional(j.at(#name).get<int>()):std::nullopt

#define FROM_JSON_BOOL_OPT(j, u, name) u.name = j.contains(#name) && j[#name].is_boolean()? std::make_optional(j.at(#name).get<bool>()):std::nullopt

#define FROM_JSON_FLOAT_OPT(j, u, name) u.name = j.contains(#name) && j[#name].is_number_float()? std::make_optional(j.at(#name).get<double>()):std::nullopt

#define FROM_JSON_NUM_OPT(j, u, name) u.name = j.contains(#name) && j[#name].is_number()? std::make_optional(j.at(#name).get<long long>()):std::nullopt

#define FRON_JSON_TYPE_OPT(j, u, type, name) u.name = j.contains(#name) ? std::make_optional(j.at(#name).get<type>()) : std::nullopt
...
#define FRON_JSON_TYPE(j, u, type, name) u.name = j.contains(#name) ? j.at(#name).get<type>() : type{}
